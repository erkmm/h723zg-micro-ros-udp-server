...
void BNO080_GPIO_SPI_Initialization(void)
{
    ...
    /*-------------------------------------------------------
     * Enable GPIO clocks — STM32H7 uses AHB4, not AHB1
     *-------------------------------------------------------*/
    LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOA);  // RST  → PA3
    LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOB);  // CS   → PB1, SCK → PB10
    LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOC);  // MISO → PC2, MOSI → PC3, INT → PC0
    LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOD);  // WAKE → PD7

    /*-------------------------------------------------------
     * SPI2 Alternate Function Pins
     * PB10 → SCK  (AF5)
     * PC2  → MISO (AF5)
     * PC3  → MOSI (AF5)
     *-------------------------------------------------------*/
    ...
    /*-------------------------------------------------------
     * Control Pins — set safe default states BEFORE
     * configuring as outputs to avoid glitches
     * CS   → PB1  (output, default HIGH = deselected)
     * RST  → PA3  (output, default HIGH = not in reset)
     * WAKE → PD7  (output, default LOW, will be set HIGH in BNO080_Initialization)
     *-------------------------------------------------------*/
    LL_GPIO_SetOutputPin(BNO080_SPI_CS_PORT,   BNO080_SPI_CS_PIN);    // CS HIGH
    LL_GPIO_SetOutputPin(BNO080_RST_PORT,      BNO080_RST_PIN);       // RST HIGH
    LL_GPIO_ResetOutputPin(BNO080_PS0_WAKE_PORT, BNO080_PS0_WAKE_PIN);// WAKE LOW for now
    ...
    /*-------------------------------------------------------
     * STM32H7: Must enable SPI here since MX_SPI2_Init
     * does not call LL_SPI_Enable()
     *-------------------------------------------------------*/
...
int BNO080_Initialization(void)
{
	...
	LL_SPI_SetTransferSize(BNO080_SPI_CHANNEL, 0);
	LL_SPI_StartMasterTransfer(BNO080_SPI_CHANNEL);
...
{
	//LL_SPI_StartMasterTransfer(BNO080_SPI_CHANNEL);
	while(LL_SPI_IsActiveFlag_TXP(BNO080_SPI_CHANNEL)==RESET);
	LL_SPI_TransmitData8(BNO080_SPI_CHANNEL, data);
	
	while(LL_SPI_IsActiveFlag_RXP(BNO080_SPI_CHANNEL)==RESET);
